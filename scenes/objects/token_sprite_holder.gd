...
func set_boxed() -> void:
	sprite.show()
	sprite.position.y = Constants.TOKEN_BOXED_Y_POS
...
func set_floating() -> void:
	...
	sprite.show()
	sprite.position.y = Constants.TOKEN_FLOATING_Y_POS
...
func set_placed() -> void:
	sprite.show()
	sprite.position.y = Constants.TOKEN_PLACED_Y_POS
	sprite.material = outline_shader
	shadow.show()
...
func set_in_range() -> void:
	sprite.show()
	sprite.position.y = Constants.TOKEN_IN_RANGE_Y_POS
	sprite.material = outline_shader
	shadow.show()
...
func _ready():
	...
	position.x = Constants.CELL_SIZE.x / 2
	shadow.position.y = Constants.TOKEN_PLACED_Y_POS
