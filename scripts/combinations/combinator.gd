...
func search_combinations_for_cell(initial_token_id: String, cell_index: Vector2, board_token_ids: Array, recursive_levels:bool, token_data_provider: TokenDataProvider) -> Combination:
	CombinationClusterProcessor.evaluate_combination(initial_token_id, combinations[cell_index], board_token_ids, recursive_levels, token_data_provider)
